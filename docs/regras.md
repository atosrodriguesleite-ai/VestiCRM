...
| RN-026 | Foto é foto: lista fechada de tipos (SVG fora) na entrada E na saída; o que não é imagem sai como download inerte, nunca como página no nosso domínio | `lib/imagem-segura.ts` | `imagem-segura.test.ts` |
| RN-027 | Consulta nova em modelo com `companyId` sem filtro de loja derruba o build; exceção se declara com `escopo-ok` e a linha de base só encolhe | `escopo-tenant.test.ts` | `escopo-tenant.test.ts` |
