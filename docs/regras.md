...
| RN-037 | Conciliação por OFX: o FITID do banco impede duplicar, o casamento automático só acontece quando a resposta é única, um depósito pode pagar várias parcelas e os dois lados têm que somar igual; conciliar carimba "conferido" e nunca mexe em baixa que já existe (mas estorno solta a conciliação); a linha SEM lançamento nenhum vira lançamento na hora, pela ficha completa (mesmo validador da RN-030), já baixado e conferido, em transação serializável; linha ilegível é contada e dita; o arquivo não é guardado | `lib/financeiro/conciliacao.ts`, `lib/financeiro/ofx.ts` | `financeiro-conciliacao.test.ts` |
